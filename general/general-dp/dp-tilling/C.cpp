...
bool has_single_component(int p, int n) {
    ...
    // В корректном состоянии должно быть четное число активных требований
    // и они должны образовывать пары
    return (active % 2 == 0) && (active <= 2); // максимум 2 активных требования на компоненту
}
    ...
    for (int j = 0; j < m; ++j) {
        for (int i = 1; i < n; ++i) {
            for (int p = 0; p < (1 << profile_bits); ++p) {
                ...
                if (second_valid) dp[i][j][second_p] += dp[i-1][j][p];
            }
        }
        ...
        for (int p = 0; p < (1 << profile_bits); ++p) {
            ...
            if (next_p & (1 << 2)) {
                if (j != m - 2) dp[0][j + 1][next_p | 1] += dp[n-1][j][p];
