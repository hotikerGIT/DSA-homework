...
private:
    ...
    int query(int index, int tree_left, int tree_right, int left, int right) {
        ...
        return query(index * 2, tree_left, tree_middle, left, std::min(right, tree_middle)) +
...
signed main() {
    ...
    while (q--) {
        ...
        if (command == 'A') T.set(--a, b);
        else std::cout << T.sum(--a, --b) << std::endl;
    }
}